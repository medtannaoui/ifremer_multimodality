...
class IRSARDataset(Dataset):
    ...
    """
    def __init__(self,test=False,size=300):
...
def train_one_epoch(fabric, model, dataloader, optimizer, metrics):
    ...
    for ir, sar in tqdm(dataloader, desc="Training"):
        ...
        # Forward (UNet diffusion -> .sample)
        pred = model(ir, timestep = 0).sample
        loss = F.mse_loss(pred, sar)
...
def main(cfg: IR_SAR_Config,test=False):
    ...
    full_data = IRSARDataset(test=test, size=cfg.img_size)
    ...
    train_ds = PairedDataset(*dictio["train"])
    ...
    model = create_model().to(fabric.device)
