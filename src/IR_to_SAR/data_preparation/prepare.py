...
class PrepareDataSet:
    ...
    def __init__(
        ...
        if self.cfg.irar:
            ...
            train_df = data[data["set"] == "train"][:50]
            val_df = data[data["set"] == "val"][:50]
            test_df = data[data["set"] == "test"][:50]
            ...
            irwin_train = np.array(irwin_train); sar_train = np.array(sar_train)
            irwin_val = np.array(irwin_val); sar_val = np.array(sar_val)
            irwin_test = np.array(irwin_test); sar_test = np.array(sar_test)
            infos_train = np.array(infos_train);infos_val = np.array(infos_val); infos_test = np.array(infos_test)
            print("Shape of Train IR : ", irwin_train.shape, "Shape of Train SAR : ", sar_train.shape)
            print("Shape of Val IR : ", irwin_val.shape, "Shape of Val SAR : ", sar_val.shape)
            print("Shape of Test IR : ", irwin_test.shape, "Shape of Test SAR : ", sar_test.shape)
        ...
        if self.cfg.anggrek_test:
            ...
            for row_idx, row in tqdm(anggrek_csv.iterrows(), total=N, desc="Generating Anggrek data"):
                ir_path = row["ir_path"]
                paths = [shift_ir_path(ir_path, idx=i, step_minutes=60 if self.cfg.irar else 30) for i in indices]
        ...
        if not self.cfg.irar:
            ...
            # Mode "IRAR" : on garde les 11 canaux IRAR temporels bruts.
            for i in range(irwin_train.shape[1]):
                image_channels_train.append(irwin_train[:, i, :, :])
                image_channels_val.append(irwin_val[:, i, :, :])
                image_channels_test.append(irwin_test[:, i, :, :])
                if anggrek_test:
        ...
        # rendre irwin en 4 dimension et sar et 3 dimension
        if len(self.X_train.shape) > 4:
            self.X_train = self.X_train.squeeze()                            
            self.X_val = self.X_val.squeeze()
            self.X_test = self.X_test.squeeze()
            if anggrek_test:
                self.X_anggrek = self.X_anggrek.squeeze()
        if len(self.sar_train.shape) > 3:
            self.sar_train = self.sar_train.squeeze()
            self.sar_val = self.sar_val.squeeze()
            self.sar_test = self.sar_test.squeeze()
